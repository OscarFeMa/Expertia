...
for r in cur.fetchall():
    print(f"{r[0]:20s} EMA={r[1]:.4f} tier={r[2]} sys_fails={r[5]} know_fails={r[6]} total={r[3]}")
...
for sid, domain, current_ema in specialists:
    ...
    for ch_row in ch:
        ...
        if success:
            if quality > 0:
                size_factor = 1.0 - math.exp(-ch[1] / 5000)
                coverage_factor = min(ch[1] / 10.0, 1.0)
                trust_factor = 50 / 100.0
                efficiency = min(1.0, 1.0 / max(1, 1))
                quality_score = 0.25 * size_factor + 0.25 * min(1.0, ch[1] / 10.0) + 0.25 * 0.5 + 0.25 * 1.0
            else:
                quality_score = 0.1
