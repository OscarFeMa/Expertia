...
setlocal
cd /d "%~dp0"
set "LOG=%TEMP%\expertia-acceso-3070-v4.log"
echo [%date% %time%] Acceso Expertia 3070 v4 offline > "%LOG%"
net session >nul 2>&1
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "powershell -NoProfile -ExecutionPolicy Bypass -File 'C:\Program Files\OpenSSH\install-sshd.ps1'; & 'C:\Program Files\OpenSSH\ssh-keygen.exe' -A 2>&1 | Out-Null; & 'C:\Program Files\OpenSSH\sshd.exe' -t 2>&1 | Out-Null; if($LASTEXITCODE -eq 0){'config OK'}else{'config FALLA, ver arriba'}" >> "%LOG%" 2>&1
