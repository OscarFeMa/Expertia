...
cur = conn.execute("""
    SELECT s.domain, s.ema_score, s.tier, s.weighted_fail, s.weighted_success,
...
for r in cur.fetchall():
    print(f"{r[1]:20s} EMA={r[1]:.4f} tier={r[2]} sys_fails={r[5]} know_fails={r[6]} total={r[3]}")
    ...
    for ch_row in ch:
        ...
        if success:
            if quality > 0:
                ...
                quality_score = 0.25 * size_factor + 0.25 * min(1.0, 1.0) + 0.25 * 0.5 + 0.25 * efficiency
            else:
                quality_score = 0.1
