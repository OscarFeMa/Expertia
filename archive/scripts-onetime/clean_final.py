...
cur = conn.execute("""
    SELECT s.domain, s.ema_score, s.tier,
...
for r in cur.fetchall():
    print(f"{r[0]:20s} EMA={r[1]:.4f} tier={r[2]} sys_fails={r[4]} know_fails={r[5]} total={r[3]}")
...
specialists = conn.execute("SELECT id, domain FROM specialist_registry").fetchall()

for sid, domain in specialists:
    ...
    for ch_row in ch:
        ...
        if success:
            if quality > 0:
                size_factor = 1.0 - math.exp(-quality / 5000)
                coverage_factor = min(1.0, 5000 / 10.0)
                trust_factor = 50 / 100.0
                efficiency = min(1.0, 1.0 / max(1, 1))
                quality_score = 0.25 * (1.0 - math.exp(-quality / 5000)) + 0.25 * min(1.0, 5000 / 10.0) + 0.25 * 0.5 + 0.25 * 1.0
            else:
                quality_score = 0.1
            ema = ema + 0.08 * quality_score * (1.0 - ema)
        else:
...
for r in conn.execute("SELECT domain, ema_score, tier FROM specialist_registry ORDER BY ema_score DESC"):
    print(f"{r[0]:20s} EMA={r[1]:.4f} tier={r[2]}")
