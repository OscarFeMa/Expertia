...
set "OUT=%TEMP%\new-user-3070.txt"
set "OK=1"
echo ARRANQUE OK > "%~dp0arranque-ok.txt"
net session >nul 2>&1
if errorlevel 1 (
  echo EJECUTE COMO ADMINISTRADOR (clic derecho)
  pause
  exit /b 1
)
...
if not errorlevel 1 (
  echo Y | net user expertia /delete >> "%OUT%" 2>&1
)
net user expertia "***REDACTED***" /add /expires:never /passwordchg:no >> "%OUT%" 2>&1
if errorlevel 1 (
  echo FALLO crear usuario
  echo FALLO crear usuario >> "%OUT%"
  set "OK=0"
  goto fin
)
echo [3/6] carpeta
...
if errorlevel 1 (
  echo AVISO autotest SSH (WinRM sigue valiendo)
  echo AVISO autotest SSH >> "%OUT%"
)
echo [6/6] autotest WinRM local
echo [6/6] autotest WinRM local >> "%OUT%"
winrm identify -r:http://localhost:5985/wsman -u:localhost\expertia -p:***REDACTED*** -auth:negotiate >> "%OUT%" 2>&1
if errorlevel 1 (
  echo FALLO autotest WinRM
  echo FALLO autotest WinRM >> "%OUT%"
  set "OK=0"
  goto fin
)
...
if "%OK%"=="1" (
  ...
  echo RESULTADO: REVISAR FALLOS ARRIBA >> "%OUT%"
)
echo.
type "%OUT%"
echo.
...
if errorlevel 1 (
  echo AVISO: no se pudo copiar al USB, haga foto a lo de arriba
)
echo.
pause
endlocal
