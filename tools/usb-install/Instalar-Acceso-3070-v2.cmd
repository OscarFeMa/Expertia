...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$j=Start-Job -ScriptBlock { New-Item -ItemType Directory -Path C:\training\out-share -Force | Out-Null; icacls C:\training\out-share /inheritance:r | Out-Null; icacls C:\training\out-share /grant:r 'SYSTEM:(OI)(CI)F' 'Administradores:(OI)(CI)F' | Out-Null; try{net user trainro /delete | Out-Null}catch{}; net user trainro '***REDACTED***' /add /expires:never /passwordchg:no | Out-Null; New-SmbShare -Name ExpertiaTrain -Path C:\training\out-share -ReadAccess trainro -FolderEnumerationMode AccessBased -EncryptData $true -Description 'Estado entreno ExpertiaMath RO' -Force | Out-Null }; if(Wait-Job $j -Timeout 120){Receive-Job $j | Out-Null; 'share OK'}else{Stop-Job $j -ErrorAction SilentlyContinue; 'share TIMEOUT, omitido'}; Remove-Job $j -Force -ErrorAction SilentlyContinue" >> "%LOG%" 2>&1
