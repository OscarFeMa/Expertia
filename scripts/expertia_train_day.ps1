...
Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object { $_.CommandLine -like "*orchestrator.py*" } | ForEach-Object {
  taskkill /F /T /PID $_.ProcessId 2>&1 | Out-Null
}
Start-Sleep -Seconds 3
...
if (-not (Test-Path $dataset)) {
  & $py (Join-Path $repo "tools\build_expertia_math_dataset.py") --limit 50000 --relaxed --out $dataset
}
$baseLocal = Join-Path $trainRoot "base\phi-4-mini-reasoning"
$modelArg = $baseLocal
if (-not (Test-Path (Join-Path $baseLocal "config.json"))) { $modelArg = "microsoft/Phi-4-mini-reasoning" }
$logDir = Join-Path $trainRoot "logs"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logFile = Join-Path $logDir "train_$stamp.log"
& $trainPy (Join-Path $trainRoot "train_expertia.py") --model $modelArg --train $dataset --out (Join-Path $trainRoot "adapters\expertia-math-r16") --offload (Join-Path $trainRoot "offload") --epochs 3 --seq-len 1024 --batch 1 --accum 16 --save-steps 200 > $logFile 2>&1
